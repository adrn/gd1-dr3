...
class FitPert:
    def __init__(self, data, data_short, mw, w0_now):
        ...
        self.gd1_rv_bonaca = rv_bonaca_data[
            rv_bonaca_data.pmmem
            & rv_bonaca_data.cmdmem
            & rv_bonaca_data.vrmem
            & rv_bonaca_data.fehmem
        ]
        self.gd1_rv_bonaca_short = self.gd1_rv_bonaca[
            (self.gd1_rv_bonaca.phi1 > -43) & (self.gd1_rv_bonaca.phi1 < -25)
        ]
    ...
    def pre_fitting(self, vals):
        ...
        ##############################################
        orbit = self.mw.integrate_orbit(
            self.w0_now, dt=-1 * u.Myr, n_steps=int(self.t_int)
        )
        old_gd1 = orbit[-1]
        ...
        center = old_gd1[
            (np.abs(np.mean(old_gd1.pos.x.value) - old_gd1.pos.x.value) < 0.5)
        ...
        self.site_at_impact_w0 = gd.PhaseSpacePosition(
            pos=np.mean(old_gd1.pos), vel=[vxstream, vystream, vzstream] * u.km / u.s
        )

        ##########################################
        ## STREAM PROPERTIES BEFORE INTERACTION ##
        ##########################################
        w0_old_stream = gd.PhaseSpacePosition(pos=old_gd1.pos, vel=old_gd1.vel)
        orbit_stream = self.mw.integrate_orbit(w0_old_stream, dt=-1 * u.Myr, n_steps=30)
        self.orig_stream = orbit_stream[-1]
        self.w0_orig_stream = gd.PhaseSpacePosition(
            pos=self.orig_stream.pos, vel=self.orig_stream.vel
        )

        self.perturber_pot = gp.HernquistPotential(
            m=10**self.logm * u.Msun, c=(10**self.logcore) * u.pc, units=galactic
        )
        # perturber_pot = gp.KeplerPotential(m=10**logm*u.Msun, units=galactic)

        return self.site_at_impact_w0

    def get_cyl_rotation(
        self,
    ):  # borrowed from Adrian Price-Whelan's streampunch github repo
    ...
    def get_perturber_w0_at_impact(self):
        ...
        perturber_pos = coord.CylindricalRepresentation(rho=b, phi=psi, z=z)
    ...
    def nbody(self):
        ...
        if pert_energy.value[0] > 0:
            ...
            w0_orig_pert = gd.PhaseSpacePosition(
                pos=[x_pert, y_pert, z_pert] * u.kpc,
            ...
            particle_pot = [
                list([self.perturber_pot])
                + [NullPotential(units=self.perturber_pot.units)]
                * self.orig_stream.shape[0]
            ][0]
            ...
            ##############################
            nbody = DirectNBody(
                w0, particle_pot, external_potential=self.mw, save_all=True
            )
            total_time = int(self.t_int) + 30
            self.orbits = nbody.integrate_orbit(
                dt=1 * u.Myr, t1=0, t2=total_time * u.Myr
            )
    ...
    def loglik(self, params):
        ...
        if not current:
            ll_model, ll_model_short, ll_phi2_short, ll_data, model_dens_ratio = (
                np.nan,
                np.nan,
                np.nan,
                np.nan,
            ...
            model_window = self.current[
                (self.current.phi1.value > -65) & (self.current.phi1.value < -22)
            ]

            # evaluate the ll using KDE
            kde_phi2_model = KernelDensity(kernel="gaussian", bandwidth=0.11).fit(
                np.array([(model_window.phi1.value + 42) / 10, model_window.phi2]).T
            )
            loglike_phi2_model = kde_phi2_model.score_samples(
                np.array(
                    [(self.data.phi1.flatten() + 42) / 10, self.data.phi2.flatten()]
                ).T
            )
            loglike_phi2_model = np.sum(loglike_phi2_model)

            kde_pm1 = KernelDensity(kernel="gaussian", bandwidth=0.21).fit(
                np.array(
                    [(model_window.phi1.value + 42) / 15, model_window.pm_phi1_cosphi2]
                ).T
            )
            loglike_pm1 = kde_pm1.score_samples(
                np.array(
                    [(self.data.phi1.flatten() + 42) / 15, self.data.pm1.flatten()]
                ).T
            )
            loglike_pm1 = np.sum(loglike_pm1)

            kde_pm2 = KernelDensity(kernel="gaussian", bandwidth=0.285).fit(
                np.array([(model_window.phi1.value + 42) / 15, model_window.pm_phi2]).T
            )
            loglike_pm2 = kde_pm2.score_samples(
                np.array(
                    [(self.data.phi1.flatten() + 42) / 15, self.data.pm2.flatten()]
                ).T
            )
            loglike_pm2 = np.sum(loglike_pm2)

            kde_rv = KernelDensity(kernel="gaussian", bandwidth=0.82).fit(
                np.array([model_window.phi1.value * 5, model_window.radial_velocity]).T
            )
            loglike_rv = kde_rv.score_samples(
                np.array([self.gd1_rv_bonaca.phi1 * 5, self.gd1_rv_bonaca.Vrad]).T
            )
            ...
            # cut output of model to window
            model_window_short = self.current[
                (self.current.phi1.value > -43) & (self.current.phi1.value < -25)
            ]

            # evaluate the ll using KDE
            kde_phi2_short = KernelDensity(kernel="gaussian", bandwidth=0.11).fit(
                np.array(
                    [(model_window_short.phi1.value) / 10, model_window_short.phi2]
                ).T
            )
            loglike_phi2_short = kde_phi2_short.score_samples(
                np.array(
                    [
                        (self.data_short.phi1.flatten()) / 10,
                        self.data_short.phi2.flatten(),
                    ]
                ).T
            )
            loglike_phi2_short = np.sum(loglike_phi2_short)

            kde_pm1_short = KernelDensity(kernel="gaussian", bandwidth=0.21).fit(
                np.array(
                    [
                        (model_window_short.phi1.value) / 15,
                        model_window_short.pm_phi1_cosphi2,
                    ]
                ).T
            )
            loglike_pm1_short = kde_pm1_short.score_samples(
                np.array(
                    [
                        (self.data_short.phi1.flatten()) / 15,
                        self.data_short.pm1.flatten(),
                    ]
                ).T
            )
            loglike_pm1_short = np.sum(loglike_pm1_short)

            kde_pm2_short = KernelDensity(kernel="gaussian", bandwidth=0.285).fit(
                np.array(
                    [(model_window_short.phi1.value) / 15, model_window_short.pm_phi2]
                ).T
            )
            loglike_pm2_short = kde_pm2_short.score_samples(
                np.array(
                    [
                        (self.data_short.phi1.flatten()) / 15,
                        self.data_short.pm2.flatten(),
                    ]
                ).T
            )
            loglike_pm2_short = np.sum(loglike_pm2_short)

            kde_rv_short = KernelDensity(kernel="gaussian", bandwidth=0.82).fit(
                np.array(
                    [
                        model_window_short.phi1.value * 5,
                        model_window_short.radial_velocity,
                    ]
                ).T
            )
            loglike_rv_short = kde_rv_short.score_samples(
                np.array(
                    [self.gd1_rv_bonaca_short.phi1 * 5, self.gd1_rv_bonaca_short.Vrad]
                ).T
            )
            ...
            high_dens_model = no_spur_model[
                (no_spur_model.phi1.value < -45) & (no_spur_model.phi1.value > -54)
            ]

            low_dens_model = no_spur_model[
                (no_spur_model.phi1.value < -39) & (no_spur_model.phi1.value > -43)
            ]
            gap_ratio = (len(low_dens_model) / 4) / (len(high_dens_model) / 9 + 0.01)

            ## Calculate the apocenter and pericenter
            orbit_pert = self.mw.integrate_orbit(
                orbits[-1, 0], dt=-1 * u.Myr, n_steps=12000
            )
            pert_apo, pert_peri = (
                orbit_pert.apocenter().value,
                orbit_pert.pericenter().value,
            )
            ...
            loglike_phi2_data = kde_phi2_data.score_samples(
                np.array(
                    [(model_window.phi1.value + 42) / 10, model_window.phi2.value]
                ).T
            )
